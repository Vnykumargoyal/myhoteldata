...
const BasicDropZone = ({
  ...
  useEffect(() => {
    if (acceptedFiles && acceptedFiles.length) {
      ...
      setDefaultFiles(files.concat(acceptedFiles));
    }
  ...
  const previewUpload = (file) => {
    ...
    return IMAGE_URLS.KYC_SCREEN.KYC_BLUR;
  };
      ...
      {notShowPreview && (
        <aside>
          {previewText && showPreviews && files.length > 0 && (
            <div className={clsx(previewTextClass, classes.previewText, 'col-white')}>
              {previewText}
            </div>
          )}
          <div>
              ...
              files.map((file, idx) => (
                ...
                >
                  <div style={{ flex: '1 1 auto' }}>
                    ...
                    {showFileNamesInPreview && (
                      <div className={classes.fileSize}>
                        <span className={clsx(fileNameClass, classes.fileName, 'col-white col-text-primary-op5 ')}>{file?.name}</span>
                        <span className={clsx(fileNameClass, 'col-white')}>{getSize(file?.size)}</span>
                      </div>
                    )}
                    {/* <Typography
                      className="fs-12"
                  ...
                  <div className={clsx(classes.deleteIcon)} onClick={removeFile(file)}>
                    <img src={IMAGE_URLS.ICONS.DELETE} alt="File Deletion" />
                  </div>
                </div>
              ))}
          </div>
...
BasicDropZone.propTypes = {
  ...
  notShowPreview: PropTypes.bool,
  setDefaultFiles: PropTypes.func,
